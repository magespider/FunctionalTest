<div class="space-y-8">
    <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6" x-data="{changePassword: <?php echo old('change_password', 0); ?>}">
        ...
        <!-- Name -->
        <x-admin.wrap-field col="2" class="{{ $errors->has('name') ? 'has-error' : ''}}">
        ...
        <x-admin.wrap-field col="1" class="{{ $errors->has('email') ? 'has-error' : ''}}">
            ...
            {!! Form::email('email', null, ('required' == 'required') ? ['class' => 'form-control w-full', 'required' => 'required'] : ['class' => 'form-control w-full']) !!}
